...
case "$mode" in
  ...
  transcript)
    ...
    body=$(cat <<EOF
{"event":"recording.transcript_completed","event_ts":$(date +%s000),"payload":{"account_id":"fake-account","object":{"uuid":"fake-uuid","id":123456789,"topic":"Fake transcript test","type":2,"host_id":"fake-host","host_email":"fake@example.com","start_time":"$now","timezone":"UTC","duration":1,"recording_count":1,"recording_files":[{"id":"fake-file","meeting_id":"123456789","file_name":"fake-transcript.vtt","recording_start":"$now","recording_end":"$now","file_type":"TRANSCRIPT","file_extension":"VTT","file_size":42,"play_url":"https://example.com/fake-transcript","download_url":"https://example.com/fake-transcript","status":"completed","recording_type":"audio_transcript"}]}} ,"download_token":"fake-download-token"}
EOF
)
    ;;
  *)
